...
function Card({ paintings, currentPage }) {
    if (!paintings || paintings.length === 0) {
        return <div className="gallery">Ничего не найдено</div>;
    }
    
    const startIndex = (currentPage - 1) * 6;
    const currentPaintings = paintings.slice(startIndex, startIndex + 6);
    
    return (
        <div className="gallery">
            {currentPaintings.map((painting, index) => (
                <ArtCardItem key={index} painting={painting} />
            ))}
        </div>
    );
}
